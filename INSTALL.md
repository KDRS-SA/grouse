...
should be able to get an Authorization token using the following command:  
    ...
    `curl -v -X POST  -H 'Authorization: Basic Z3JvdXNlLWNsaWVudDpzZWNyZXQ=' http://localhost:9294/grouse/oauth/token -d grant_type=password -d username=user@kdrs.no -d password=password`
